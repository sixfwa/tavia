...
Database.initialise(user='tavia', password='1234',
                    host='localhost', database='tavia')
...
if not user:
    ...
    # Get the request token parsing the the query string returned
    ...
    print("{}?oauth_token={}".format(
        constants.AUTHORIZATION_URL, request_token['oauth_token']))
    ...
    # Ask Twitter for an access token, and Twitter knows it should give us it because we've verified the
    # request token
    ...
    user = User(user_email, first_name, last_name,
...
response, content = authorized_client.request(
    'https://api.twitter.com/1.1/search/tweets.json?q=computers', 'GET')
